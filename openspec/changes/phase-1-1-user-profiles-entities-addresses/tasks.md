...
- [ ] 1.1 Create migration `create_user_profiles_table` with columns: id, user_id (FK), jurisdiction_id (FK), tax_id, status, timestamps
  - Include unique index on (user_id, jurisdiction_id)
  - Foreign keys: ON DELETE RESTRICT, ON UPDATE CASCADE
  - → Tests passing: 20.1, 20.2

- [ ] 1.2 Create migration `create_entities_table` with columns: id, user_profile_id (FK), name, entity_type (string or enum cast), tax_id, status, timestamps
  - Foreign key user_profile_id: ON DELETE CASCADE
  - → Tests passing: 20.3

- [ ] 1.3 Create migration `create_addresses_table` with columns: id, addressable_id, addressable_type, user_id (FK), street, city, state, postal_code, country, timestamps
...
- [ ] 2.1 Create `app/Enums/EntityType.php` enum with cases: LLC, SCorp, CCorp, Partnership, Trust, Other (no Individual)
  - Keys in TitleCase
...
- [ ] 3.1 Create `app/Models/UserProfile.php` model
...
- [ ] 3.2 Add casts() method to UserProfile: user_id → int, jurisdiction_id → int, status → string
  - → Tests passing: 22.1

- [ ] 3.3 Add relationships to UserProfile:
...
- [ ] 3.4 Create `database/factories/UserProfileFactory.php`
...
- [ ] 3.5 Add UserProfile relationship to User model: `userProfiles(): HasMany`
  - → Tests passing: 23.1
...
- [ ] 4.1 Create `app/Models/Entity.php` model
  - PHPDoc block with `@property-read` for all attributes
...
- [ ] 4.2 Add casts() method to Entity: user_profile_id → int, entity_type → EntityType (enum), status → string
  - EntityType enum cast ensures type safety
  - → Tests passing: 6.2, 22.1

- [ ] 4.3 Add relationships to Entity:
  - `userProfile(): BelongsTo` → UserProfile model
  - `address(): MorphOne` → Address model
  - → Tests passing: 7.2, 10.1, 10.2, 23.6, 23.7

- [ ] 4.4 Create `database/factories/EntityFactory.php`
...
- [ ] 4.5 Add validation logic (defer to Form Requests in Fase 4, but factories should validate structure)
...
- [ ] 5.1 Create `app/Models/Address.php` model
  - PHPDoc block with `@property-read` for all attributes
...
- [ ] 5.2 Add casts() method to Address: addressable_id → int, user_id → int, and all string fields
  - → Tests passing: 22.1

- [ ] 5.3 Add relationships to Address:
...
- [ ] 5.4 Create `database/factories/AddressFactory.php`
...
- [ ] 6.1 Verify User model has relationship to UserProfile (already should exist from step 3.5)
  - Test: `user.userProfiles()`
  - → Tests passing: 23.1

- [ ] 6.2 Verify all eager-loading relationships work without N+1
...
- [ ] 6.3 Verify polymorph inverse (Address.user accessing addresses)
...
- [ ] 7.1 Write tests for UserProfile creation, update, deletion
...
- [ ] 8.1 Write feature tests for UserProfile routes (defer controller creation to Fase 4, but define route expectations)
...
- [ ] 8.2 Write feature tests for Entity routes
  - Tests 6.6, 6.7, 7.4, 7.5
  - Tests may SKIP if routes/controllers not yet created (note as "deferred to Fase 4")
  - → Tests passing: 6.6, 6.7, 7.4, 7.5

- [ ] 8.3 Write feature tests for Address routes
...
- [ ] 9.1 Write integration tests for full data flow (User → Profile → Entity → Address)
...
- [ ] 10.1 Run `vendor/bin/pint --dirty` to format PHP code
  - Ensure all models, factories, migrations follow project style
  - → All PHP files formatted

- [ ] 10.2 Run `vendor/bin/rector --dry-run` to check for Rector refactoring opportunities
  - Fix any suggested issues (or accept intentional patterns)
  - → Rector checks pass

- [ ] 10.3 Run `composer lint` (full linting suite)
...
- [ ] 11.1 Run full test suite: `php artisan test --compact`
  - All tests must PASS (unit, feature, integration)
  - → All ~75 tests passing

- [ ] 11.2 Verify type coverage: `php artisan test --type-coverage --compact`
  - 100% type coverage enforced
  - → Type coverage = 100%

- [ ] 11.3 Run composer test (full verification)
  - Includes: Pest, PHPUnit, Pint, Rector, Larastan, type coverage
  - → All checks pass

- [ ] 11.4 Clean up test data / seeders if created for development
...
- [ ] 12.1 Add PHPDoc comments to all models, factories, migrations
  - Document relationships, enum use, polymorph patterns
  - → Code is well-documented

- [ ] 12.2 Verify no leftover debugging code, debug statements, commented-out code
  - → Code is clean

- [ ] 12.3 Create short README or migration notes (optional, if complex)
...
- [ ] 13.1 Extract any shared validation logic into traits (if needed)
  - E.g., if UserProfile and Entity share similar field requirements
  - → Code reuse increased

- [ ] 13.2 Consider adding Concerns for common behaviors
...
- [ ] 14.1 All 5 OpenSpec artifacts complete (proposal, design, specs, tests, tasks)
- [ ] 14.2 All tests PASSING (unit, feature, integration, browser-skipped)
- [ ] 14.3 100% type coverage
- [ ] 14.4 All code formatted and linted
- [ ] 14.5 No N+1 queries (verified in tests)
- [ ] 14.6 Database migrations reversible (tested in rollback)
- [ ] 14.7 Ready for `/opsx:archive` or `/opsx:apply` to implement
